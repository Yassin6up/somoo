...
import {
    ...
    type GroupTaskAnnouncement,
    type InsertGroupTaskAnnouncement,
    type GroupPost,
...
export interface IStorage {
    ...
    // Group Task Announcements
    getGroupTaskAnnouncements(groupId: string): Promise<GroupTaskAnnouncement[]>;
    createGroupTaskAnnouncement(announcement: InsertGroupTaskAnnouncement): Promise<GroupTaskAnnouncement>;
    volunteerForGroupTask(announcementId: string, freelancerId: string): Promise<GroupTaskAnnouncement | undefined>;
}
...
export class DatabaseStorage implements IStorage {
    ...
    // Group Task Announcements methods
    async getGroupTaskAnnouncements(groupId: string): Promise<GroupTaskAnnouncement[]> {
        return await db
            .select()
            .from(groupTaskAnnouncements)
            .where(eq(groupTaskAnnouncements.groupId, groupId))
            .orderBy(desc(groupTaskAnnouncements.createdAt));
    }

    async createGroupTaskAnnouncement(announcement: InsertGroupTaskAnnouncement): Promise<GroupTaskAnnouncement> {
        const [created] = await db
            .insert(groupTaskAnnouncements)
            .values(announcement)
            .returning();
        return created;
    }

    async volunteerForGroupTask(announcementId: string, freelancerId: string): Promise<GroupTaskAnnouncement | undefined> {
        const [existing] = await db
            .select()
            .from(groupTaskAnnouncements)
            .where(eq(groupTaskAnnouncements.id, announcementId))
            .limit(1);
        if (!existing) return undefined;
        const current: string[] = Array.isArray(existing.volunteers) ? existing.volunteers : [];
        if (current.includes(freelancerId)) return existing; // already volunteered
        const updatedList = [...current, freelancerId];
        const [updated] = await db
            .update(groupTaskAnnouncements)
            .set({
                volunteers: updatedList as any,
                volunteersCount: updatedList.length,
            })
            .where(eq(groupTaskAnnouncements.id, announcementId))
            .returning();
        return updated || undefined;
    ...
    async getDirectMessageHistory(
        ...
        console.log("getDirectMessageHistory called with:", { userId, userType });
        ...
        console.log("Sent query params:", { senderId: userId, senderType: userType });
        ...
        console.log("Received query params:", { receiverId: userId, receiverType: userType });
        ...
        console.log("Unique conversations count:", result.length);
        console.log("Unique conversations:", result);
    ...
    async holdInEscrow(productOwnerId: string, amount: number, proposalId: string, description: string): Promise<ProductOwnerWallet> {
        const wallet = await this.getOrCreateProductOwnerWallet(productOwnerId);

        const availableBalance = parseFloat(wallet.availableBalance);
        const escrowBalance = parseFloat(wallet.escrowBalance);

        if (availableBalance < amount) {
            throw new Error("رصيد غير كافٍ");
        }

        const newAvailableBalance = (availableBalance - amount).toFixed(2);
        const newEscrowBalance = (escrowBalance + amount).toFixed(2);
        ...
        const [updated] = await db
            ...
            .where(eq(productOwnerWallets.id, wallet.id))
            .returning();
