...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    app.get("/api/freelancers/:id/groups", authMiddleware, requireRole(["freelancer"]), async (req: AuthRequest, res) => {
        try {
            ...
            if (req.user?.userId !== id) {
                return res.status(403).json({ error: "ليس لديك صلاحية لعرض هذه المجموعات" });
            }
    ...
    app.get("/api/product-owners/:id/wallet", authMiddleware, async (req: AuthRequest, res) => {
        try {
            ...
            console.log("📥 Fetching wallet for product owner:", id);
            console.log("👤 Requesting user:", req.user?.userId, req.user?.userType);

            // Verify user can only access their own wallet
            if (req.user?.userId !== id && req.user?.userType !== "admin") {
                console.log("❌ Unauthorized access attempt");
                return res.status(403).json({ error: "ليس لديك صلاحية لعرض هذه المحفظة" });
            }
    ...
    app.get("/api/product-owners/:id/wallet/transactions", authMiddleware, async (req: AuthRequest, res) => {
        try {
            ...
            console.log("📥 Fetching wallet transactions for product owner:", id);

            // Verify user can only access their own transactions
            if (req.user?.userId !== id && req.user?.userType !== "admin") {
                return res.status(403).json({ error: "ليس لديك صلاحية لعرض هذه المعاملات" });
            }
            ...
            console.log("✅ Returning transactions:", transactionsList.length);
...
ساعده في تحسين هذا الوصف ليكون أكثر وضوحًا وجاذبية للمختبرين. اكتب نسخة محسّنة من الوصف بالعربية، في 2-3 جمل فقط، مع التركيز على الميزات الرئيسية والقيمة المضافة.`;
    ...
    app.get("/api/projects/my", authMiddleware, requireRole(["product_owner"]), async (req: AuthRequest, res) => {
        try {
            ...
            res.status(500).json({ error: "حدث خطأ أثناء جلب مشاريعي" });
        }
    });
    ...
    app.post("/api/projects/:id/accept", authMiddleware, requireRole(["freelancer"]), async (req: AuthRequest, res) => {
        try {
            ...
            if (!leaderId || !groupId) {
                return res.status(400).json({ error: "معرف القائد ومعرف المجموعة مطلوبان" });
            }
            ...
            if (group.leaderId !== leaderId) {
                return res.status(403).json({ error: "فقط قائد المجموعة يمكنه قبول المشروع" });
            }
            ...
            if (project.status !== "pending") {
                return res.status(400).json({ error: "لا يمكن قبول هذا المشروع في حالته الحالية" });
            }

            // Accept the project
            const updatedProject = await storage.acceptProject(id, groupId);
            if (!updatedProject) {
                return res.status(500).json({ error: "فشل قبول المشروع" });
            }
            ...
            await storage.createNotification({
                ...
                message: `قبل الفريق \"${group.name}\" مشروعك \"${project.title}\"`,
    ...
    app.get("/api/projects/group/:groupId", authMiddleware, async (req: AuthRequest, res) => {
        try {
            ...
            if (!isLeader && !isMember) {
                return res.status(403).json({ error: "ليس لديك صلاحية لعرض مشاريع هذه المجموعة" });
            }
            ...
            res.status(500).json({ error: "حدث خطأ أثناء جلب مشاريع المجموعة" });
        }
    });
        ...
        try {
            ...
            if (!orderId && !projectId) {
                return res.status(400).json({
                    error: "❌ لا توجد طلبات نشطة - تحتاج إلى طلب نشط أو مشروع مقبول لإنشاء مهام."
                });
            }
            ...
            if (orderId) {
                ...
                if (order.status !== "pending") {
                    return res.status(400).json({ error: "الطلب ليس في حالة معلقة" });
                }
            }
            ...
            if (projectId) {
                ...
                if (project.acceptedByGroupId !== groupId) {
                    return res.status(403).json({ error: "هذا المشروع غير مقبول من قبل مجموعتك" });
                }
            }
    ...
    app.get("/api/groups/:groupId/tasks", authMiddleware, async (req: AuthRequest, res) => {
        try {
            ...
            const isMember = await storage.isGroupMember(groupId, requesterId);
            if (!isLeader && !isMember) {
                return res.status(403).json({ error: "ليس لديك صلاحية لعرض مهام هذه المجموعة" });
            }
            ...
            res.status(500).json({ error: "حدث خطأ أثناء جلب مهام المجموعة" });
        }
    });
    ...
    app.get("/api/groups/:id/spectator-status", authMiddleware, requireRole(["product_owner"]), async (req: AuthRequest, res) => {
        try {
            ...
            res.status(500).json({ error: "تعذر جلب حالة المشاهدة" });
        }
    });
    ...
    app.get("/api/groups/:id/task-announcements", authMiddleware, async (req: AuthRequest, res) => {
        try {
            ...
            if (req.user.role === "product_owner") {
                ...
                if (!ok) return res.status(403).json({ error: "غير مصرح (يجب أن تكون مشاهد)" });
                ...
                if (!isMember && group.leaderId !== req.user.userId) return res.status(403).json({ error: "غير مصرح (عضو أو قائد فقط)" });
            }
            const items = await storage.getGroupTaskAnnouncements(groupId);
            res.json(items);
        } catch (error) {
            console.error("Error list task-announcements:", error);
            res.status(500).json({ error: "تعذر جلب إعلانات المهام" });
        }
    });
    ...
    app.post("/api/groups/:id/task-announcements", authMiddleware, requireRole(["freelancer"]), async (req: AuthRequest, res) => {
        try {
            ...
            const group = await storage.getGroup(groupId);
            if (!group) return res.status(404).json({ error: "الجروب غير موجود" });
            if (group.leaderId !== req.user.userId) return res.status(403).json({ error: "فقط القائد يمكنه النشر" });
            ...
            await Promise.all(spectators.map(s => storage.createNotification({ userId: s.productOwnerId, userType: "product_owner", title: "إعلان مهمة جديد", message: `تم نشر مهمة: ${title}`, type: "task_announcement" })));
    ...
    app.post("/api/groups/:id/task-announcements/:announcementId/volunteer", authMiddleware, requireRole(["freelancer"]), async (req: AuthRequest, res) => {
        try {
            ...
            if (!isMember && !isLeader) return res.status(403).json({ error: "غير مصرح (عضو أو قائد فقط)" });
            ...
            res.status(500).json({ error: "تعذر التطوع للمهمة" });
        }
    });
        ...
        try {
            ...
            if (status === "approved") {
                ...
                await storage.createNotification({
                    ...
                    message: `تم قبول طلب انضمامك لجروب "${group.name}". أنت الآن عضو في الجروب!`,
        ...
        try {
            ...
            if (isGroupLeader && !isProductOwner) {
                if (Object.keys(updates).length !== 1 || updates.status !== "completed") {
                    return res.status(403).json({ error: "قائد المجموعة يمكنه فقط تحديث حالة المشروع إلى مكتمل" });
                }
            }
    ...
    app.post("/api/proposals/accept", authMiddleware, async (req: AuthRequest, res) => {
      try {
        ...
        if (availableBalance < budgetAmount) {
          return res.status(400).json({ 
            error: `رصيدك غير كافٍ. الرصيد المتاح: ${availableBalance} ريال، المطلوب: ${budgetAmount} ريال` 
          });
        }
        ...
        io.to(`user:${leaderId}`).emit('notification', {
          title: 'تم قبول عرضك!',
          message: `قام ${req.user!.userId} بقبول عرض المشروع: ${title}`,
        ...
        console.error("Error accepting proposal:", error);
        res.status(500).json({ error: "حدث خطأ أثناء قبول العرض" });
      }
    });
    ...
    app.post("/api/proposals/reject", authMiddleware, async (req: AuthRequest, res) => {
      try {
        ...
        io.to(`user:${leaderId}`).emit('notification', {
          title: 'تم رفض عرضك',
          message: `قام صاحب المشروع برفض عرضك: ${proposalData.title}`,
        ...
        console.error("Error rejecting proposal:", error);
        res.status(500).json({ error: "حدث خطأ أثناء رفض العرض" });
      }
    });
    ...
    app.get("/api/wallet", authMiddleware, async (req: AuthRequest, res) => {
      try {
        ...
        console.error("Error fetching wallet:", error);
        res.status(500).json({ error: "حدث خطأ في جلب المحفظة" });
      }
    });
    ...
    app.post("/api/wallet/add-funds", authMiddleware, async (req: AuthRequest, res) => {
      try {
        ...
        res.json({ success: true, message: `تم إضافة ${amount} ريال إلى محفظتك` });
      } catch (error) {
        console.error("Error adding funds:", error);
        res.status(500).json({ error: "حدث خطأ في إضافة الرصيد" });
      }
    });
    */
    ...
    app.post("/api/proposals/accept", authMiddleware, async (req: AuthRequest, res) => {
        ...
        console.log('👤 User:', { userId: req.user?.userId, userType: req.user?.userType });
        ...
        try {
            ...
            if (userType !== "product_owner") {
                console.log('❌ User is not a product owner');
                return res.status(403).json({ error: "فقط أصحاب المشاريع يمكنهم قبول العروض" });
            }

            console.log("✅ User type verified");
            console.log("🔍 Checking conversation type...");
            ...
            if (!conversation) {
                console.log('⚠️ No conversation found - this is a direct message');
                console.log('💡 For direct messages, we will use a simplified flow without conversation record');
                ...
                console.log('✅ Using direct message flow');
                ...
                console.log('🔨 Creating conversation record for direct message...');
                try {
            ...
            console.log('👥 Group ID:', groupId || 'No group (individual freelancer)');
            ...
            if (groupId) {
                ...
                console.log('👥 Group:', group ? `Found: ${group.name}` : 'Not found');

                if (group && group.leaderId === leaderId) {
                    isGroupLeader = true;
                    console.log('✅ Leader has a group');
                } else {
                    console.log('ℹ️ Not a group leader or group not found');
                }
            }
            ...
            if (parseFloat(wallet.availableBalance) < budgetAmount) {
                ...
                return res.status(400).json({
                    error: `رصيدك غير كافٍ. الرصيد المتاح: ${parseFloat(wallet.availableBalance).toFixed(2)} ريال، المطلوب: ${budgetAmount.toFixed(2)} ريال`,
            ...
            console.log('👥 Is Group Leader:', isGroupLeader, 'Group ID:', groupId);
            ...
            console.log('🔒 Holding money in escrow...');
            // Now hold money in escrow with the proposal ID
            await storage.holdInEscrow(productOwnerId, budgetAmount, proposal.id, `عرض المشروع: ${title}`);
            console.log('✅ Money held in escrow');

            // If this is a group acceptance ensure product owner is spectator and notify group members
            if (isGroupLeader && groupId) {
                try {
                    console.log('👀 Ensuring product owner spectator status for group:', groupId);
                    ...
                    if (!alreadySpectator) {
                        ...
                        console.log('✅ Product owner added as spectator');
                        await storage.createNotification({
                            userId: productOwnerId,
                            userType: 'product_owner',
                            title: 'تم تفعيل وضع المشاهدة',
                            message: `يمكنك الآن متابعة مجتمع الجروب "${group?.name || ''}" في وضع المشاهدة دون نشر أو تعليق`,
                            type: 'spectator_added'
                        });
                    } else {
                        console.log('ℹ️ Product owner already spectator');
                    }

                    // Notify all group members (including leader) about project acceptance
                    const members = await storage.getGroupMembers(groupId);
                    console.log(`🔔 Notifying ${members.length} group members about accepted project`);
                    await Promise.all(members.map(m => storage.createNotification({
                        userId: m.freelancerId,
                        userType: 'freelancer',
                        title: 'مشروع جديد قيد التنفيذ',
                        message: `تم قبول عرض المشروع "${title}". استعدوا للمهام القادمة!`,
                        type: 'project_accepted'
                    })));
                } catch (spectErr) {
                    console.error('❌ Error handling spectator / member notifications:', spectErr);
                }
            }
            ...
            if (!isDirectMessage) {
                ...
                await storage.sendMessage(
                    ...
                    productOwnerId,
                    "product_owner",
                    `✅ تم قبول العرض! تم إنشاء المشروع "${title}" وحجز ${budget} ريال في الضمان.`
                );
            ...
            io.to(`user:${leaderId}`).emit('notification', {
                title: 'تم قبول عرضك!',
                message: `تم قبول عرض المشروع "${title}" من قبل صاحب العمل`,
                type: 'proposal_accepted',
                projectId: project.id,
            });
            ...
            if (isGroupLeader && group) {
                ...
                responseData.message = `تم قبول العرض وإنشاء المشروع. يمكن لقائد المجموعة الآن إنشاء مهام للأعضاء. سيتم توزيع ${membersShare.toFixed(2)} ريال على الأعضاء تلقائياً عند إكمال المهام.`;
            } else {
                responseData.message = 'تم قبول العرض وإنشاء المشروع. يمكنك البدء في العمل مباشرة.';
            }
            ...
            console.error("🔴 ========== ERROR IN ACCEPT PROPOSAL ==========");
            console.error("💥 Error:", error);
            console.error("📚 Stack:", error instanceof Error ? error.stack : 'No stack trace');
            console.error("🔴 ================================================\n");
            res.status(500).json({ error: "حدث خطأ في قبول العرض" });
        }
    });
    ...
    app.post("/api/proposals/reject", authMiddleware, async (req: AuthRequest, res) => {
        try {
            ...
            if (userType !== "product_owner") {
                return res.status(403).json({ error: "فقط أصحاب المشاريع يمكنهم رفض العروض" });
            }
            ...
            await storage.sendMessage(
                ...
                productOwnerId,
                "product_owner",
                `❌ تم رفض العرض: ${proposalData.title}`
            );

            // Send notification to leader
            io.to(`user:${leaderId}`).emit('notification', {
                title: 'تم رفض عرضك',
                message: `تم رفض عرض المشروع "${proposalData.title}"`,
                type: 'proposal_rejected',
            });
            ...
            console.error("Error rejecting proposal:", error);
            res.status(500).json({ error: "حدث خطأ في رفض العرض" });
        }
    });
    ...
    app.post("/api/projects/:projectId/tasks", authMiddleware, async (req: AuthRequest, res) => {
        try {
            ...
            // Verify user is the group leader
            if (userType !== "freelancer") {
                return res.status(403).json({ error: "فقط قادة الجروبات يمكنهم إنشاء المهام" });
            }
            ...
            if (!group || group.leaderId !== userId) {
                return res.status(403).json({ error: "فقط قائد الجروب يمكنه إنشاء مهام لهذا المشروع" });
            }
            ...
            if (memberCount === 0) {
                return res.status(400).json({ error: "لا يوجد أعضاء في الجروب لتوزيع المهام" });
            }

            // Calculate automatic payment distribution
            const projectBudget = parseFloat(project.budget);
            const leaderShare = projectBudget * 0.4; // 40% للقائد
            const membersPool = projectBudget * 0.6; // 60% للأعضاء
            const taskReward = membersPool / memberCount; // تقسيم متساوي بين الأعضاء
            ...
            io.to(`user:${assignedTo}`).emit('notification', {
                title: 'مهمة جديدة',
                message: `تم تعيينك لمهمة "${title}" بمكافأة ${netReward.toFixed(2)} ريال`,
                type: 'task_assigned',
            });
    ...
    app.post("/api/proposals/create", authMiddleware, requireRole(["freelancer"]), async (req: AuthRequest, res) => {
        try {
            ...
            if (group.leaderId !== user.userId) {
                return res.status(403).json({ error: "فقط قائد المجموعة يمكنه إنشاء مقترحات" });
            }
            ...
            io.to(`user:${proposal.productOwnerId}`).emit('notification', {
                ...
                message: `تلقيت مقترح جديد: ${proposal.title}`,
