...
export function PostCard({
  ...
  // Check if current user is product owner (visitor)
  ...
  const handleCommentSubmit = (e: React.FormEvent) => {
    ...
    // Only require image if post has a task and task is not completed
    if (hasTask && !allApproved && !imageUrl) {
      toast({
        title: "صورة مطلوبة",
    ...
    // For regular posts, allow comments without content if image exists, or with content
    if (!hasTask && !content?.trim() && !imageUrl) {
      toast({
        title: "تنبيه",
        description: "يجب كتابة تعليق أو إرفاق صورة",
        variant: "destructive",
      });
      return;
    }
    ...
    <Card
      ...
      <CardContent className="p-4">
        ...
        <div className="flex items-center justify-between border-t border-b border-gray-200 py-2 mb-3">
          <Button
            variant="ghost"
            className={`flex-1 gap-2 hover:bg-gray-50 ${userReaction ? 'text-blue-600' : 'text-gray-600'} ${isProductOwner ? 'opacity-50 cursor-not-allowed' : ''}`}
            onClick={() => !isProductOwner && toggleReactionMutation.mutate()}
                      ...
                      <div className="bg-gray-50 rounded-lg px-3 py-2 inline-block">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {comment.isTaskCompleted && (
                            <Badge className="bg-gradient-to-r from-green-500 to-emerald-600 text-white px-2 py-0.5 text-xs border-0">
                              <Award className="w-3 h-3 ml-1" />
                              مهمة مكتملة
                              {comment.taskCompletionReward && (
                                <span className="mr-1">• ${comment.taskCompletionReward}</span>
                              )}
                            </Badge>
                          )}
                        </div>
            ...
            <div className="flex gap-2 items-start">
              ...
              <div className="flex-1">
                ...
                <div className="absolute left-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
                  ...
                  <button
                    type="submit"
                    disabled={createCommentMutation.isPending || (!hasTask && !commentInputs[post.id]?.trim() && !commentImages[post.id])}
